...
public class RelativePos { // RelativePos are stored as literal integers, meaning 1 for 1, not 0 for 1
							// They used to be stored as start-at-0 integers, although when converting to
							// absolute, 1 integer was lost for every chunk
							// that was present within the position
	...
	public int compare(RelativePos testRelative) { // returns the value of testRelative in relation to this
		if (testRelative.chunkPoint.x <= 0 || testRelative.chunkPoint.y <= 0 || testRelative.tilePoint.getX() < 0
			...
			return -2;
		} else if (testRelative.chunkPoint.equals(chunkPoint) && testRelative.tilePoint.equals(tilePoint)) { // 0 = this
																												// rel
																												// and
																												// testRel
																												// equal
	...
	public static Point3D correctOutOfBounds(Point3D pos) {
		...
		if (generator().compare(toRel(pos)) == 1) { // Correcting for greater than bounds positions
			...
			Point3D maxModPos = new Point3D(pos.getX() % generatorAbs.getX(), pos.getY() % generatorAbs.getY(),
					pos.getZ() % generatorAbs.getZ()); // remainder from the point being divided by the map's absolute
														// dimensions
			double correctX = maxModPos.getX() == copyPos.getX() ? copyPos.getX()
					: (copyPos.getX() - maxModPos.getX()) / ((copyPos.getX() - maxModPos.getX()) / generatorAbs.getX()); // only
																															// subtract
																															// the
																															// mod
																															// if
			...
			double correctY = maxModPos.getY() == copyPos.getY() ? copyPos.getY()
					: (copyPos.getY() - maxModPos.getY()) / ((copyPos.getY() - maxModPos.getY()) / generatorAbs.getY()); // and
																															// make
																															// sure
																															// to
																															// correct
			...
			Point3D absValPos = new Point3D(Math.abs(pos.getX()), Math.abs(pos.getY()), Math.abs(pos.getZ())); // Adding
																												// the
																												// absolute
																												// val
																												// of a
			...
			copyPos = new Point3D(copyPos.getX() + absValPos.getX(), copyPos.getY() + absValPos.getY(),
					copyPos.getZ() + absValPos.getZ()); // Negative values in copypos are now 0, but positive values are
														// now doubled
			copyPos = new Point3D(copyPos.getX() / 2, copyPos.getY() / 2, copyPos.getZ() / 2); // The doubled positive
																								// integers are now
																								// halved and normal,
																								// but the 0 values are
																								// unaffected
	...
	public static RelativePos toRel(Point3D absPoint) {
		Point chunkLoc = new Point((int) Math.floor(absPoint.getX() / generator.tilePoint.getX() + 1), // Finds out what
																										// chunk the
																										// absPoint
																										// falls in
				(int) Math.floor(absPoint.getY() / generator.tilePoint.getY()) + 1); // Can't use ceiling because the
																						// ceiling of 0 is 0
		Point3D tileLoc = new Point3D(absPoint.getX() + 1 - (generator.tilePoint.getX() * (chunkLoc.x - 1)), // Cuts off
																												// the
																												// amount
																												// of
																												// tiles
